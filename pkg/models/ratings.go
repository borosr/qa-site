...
var RatingWhere = struct {
	...
	Kind:     whereHelperstring{field: "\"ratings\".\"kind\""},
	RecordID: whereHelperstring{field: "\"ratings\".\"record_id\""},
	RatedBy:  whereHelperstring{field: "\"ratings\".\"rated_by\""},
	RatedAt:  whereHelpertime_Time{field: "\"ratings\".\"rated_at\""},
	Value:    whereHelperint16{field: "\"ratings\".\"value\""},
}
...
type ratingR struct {
	RatedByUser *User `boil:"RatedByUser" json:"RatedByUser" toml:"RatedByUser" yaml:"RatedByUser"`
}
...
var (
	ratingAllColumns            = []string{"id", "kind", "record_id", "rated_by", "rated_at", "value"}
	ratingColumnsWithoutDefault = []string{"id", "kind", "record_id", "rated_by", "rated_at", "value"}
	ratingColumnsWithDefault    = []string{}
	ratingPrimaryKeyColumns     = []string{"id"}
)

type (
	// RatingSlice is an alias for a slice of pointers to Rating.
	// This should generally be used opposed to []Rating.
	RatingSlice []*Rating
...
// LoadRatedByUser allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (ratingL) LoadRatedByUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRating interface{}, mods queries.Applicator) error {
	var slice []*Rating
	var object *Rating

	if singular {
		object = maybeRating.(*Rating)
	} else {
		slice = *maybeRating.(*[]*Rating)
	}
	...
	if singular {
		...
		args = append(args, object.RatedBy)
	...
	if singular {
		...
		object.R.RatedByUser = foreign
		if foreign.R == nil {
			foreign.R = &userR{}
		}
		foreign.R.RatedByRatings = append(foreign.R.RatedByRatings, object)
		return nil
	}

	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.RatedBy == foreign.ID {
				local.R.RatedByUser = foreign
				if foreign.R == nil {
					foreign.R = &userR{}
				}
				foreign.R.RatedByRatings = append(foreign.R.RatedByRatings, local)
				break
			}
		}
	}
...
// SetRatedByUser of the rating to the related item.
// Sets o.R.RatedByUser to related.
// Adds o to related.R.RatedByRatings.
func (o *Rating) SetRatedByUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	...
	o.RatedBy = related.ID
	if o.R == nil {
		o.R = &ratingR{
			RatedByUser: related,
		}
	} else {
		o.R.RatedByUser = related
	}

	if related.R == nil {
		related.R = &userR{
			RatedByRatings: RatingSlice{o},
		}
	} else {
		related.R.RatedByRatings = append(related.R.RatedByRatings, o)
	}
...
func Ratings(mods ...qm.QueryMod) ratingQuery {
	...
	return ratingQuery{NewQuery(mods...)}
}
...
func (o *Rating) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryCockroachDB(dialect, "\"ratings\"", updateOnConflict, ret, update, conflict, insert)
