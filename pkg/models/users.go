...
func (o *User) CreatedByAnswers(mods ...qm.QueryMod) answerQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"answers\".\"created_by\"=?", o.ID),
	)
...
func (o *User) CreatedByQuestions(mods ...qm.QueryMod) questionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"questions\".\"created_by\"=?", o.ID),
	)
...
func (userL) LoadCreatedByAnswers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`answers`),
		qm.WhereIn(`answers.created_by in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.CreatedByAnswers = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.CreatedBy {
				local.R.CreatedByAnswers = append(local.R.CreatedByAnswers, foreign)
				if foreign.R == nil {
					foreign.R = &answerR{}
				}
...
// LoadCreatedByQuestions allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadCreatedByQuestions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`questions`),
		qm.WhereIn(`questions.created_by in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.CreatedByQuestions = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &questionR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.CreatedBy {
				local.R.CreatedByQuestions = append(local.R.CreatedByQuestions, foreign)
				if foreign.R == nil {
					foreign.R = &questionR{}
				}
				foreign.R.CreatedByUser = local
...
func (o *User) AddCreatedByAnswers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Answer) error {
	...
	if o.R == nil {
		o.R = &userR{
			CreatedByAnswers: related,
		}
	} else {
		o.R.CreatedByAnswers = append(o.R.CreatedByAnswers, related...)
	}
...
// AddCreatedByQuestions adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CreatedByQuestions.
// Sets related.R.CreatedByUser appropriately.
func (o *User) AddCreatedByQuestions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Question) error {
	...
	if o.R == nil {
		o.R = &userR{
			CreatedByQuestions: related,
		}
	} else {
		o.R.CreatedByQuestions = append(o.R.CreatedByQuestions, related...)
	}
